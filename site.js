...
// ── THEME ──────────────────────────────────────────────────────────────────
// Four palettes on one button, cycling light → dark → fond → fondwave.
//
...
// The fourth is Fondwave — the same warm, shadowed, vine-decorated structure
// as light/dark, just recoloured from the "Fondwave" Plasma colour scheme
// (ivory and indigo instead of parchment and brick-red).
//
// Fond differs from the rest in one way that matters here — it follows the
// operating system's light/dark setting rather than fixing one, exactly as
// every app in the suite does (System is their honoured default). Fondwave,
// like light and dark, is a fixed palette with no OS-following face.
//
...
(function () {
  ...
  const ORDER = ['light', 'dark', 'fond', 'fondwave'];
  const FACE = { light: ['☀️', 'Light'], dark: ['🌙', 'Dark'], fond: ['❖', 'Fond'], fondwave: ['🌊', 'Fondwave'] };
  // The address-bar colour, per theme. Fond's pair is its ground in each scheme.
  const BAR = { light: '#e8d5b0', dark: '#6a1710', fondLight: '#f3f3f5', fondDark: '#28282c', fondwave: '#e8dac8' };

  const root = document.documentElement;
  const meta = document.getElementById('themeColorMeta');
  const osDark = window.matchMedia('(prefers-color-scheme: dark)');

  // What light/dark actually resolves to — the warm themes and Fondwave say so
  // themselves, Fond asks the OS. Anything keyed to brightness rather than to
  // palette (the screenshots, most obviously) has to follow this and not the
  // theme name — Fondwave resolves to its own name here, which reads as
  // "light" to anything doing a `=== 'dark'` brightness check.
  ...
  const setTheme = t => {
    ...
    btn.title = 'Theme: ' + name + ' — click for ' + FACE[ORDER[(ORDER.indexOf(t) + 1) % ORDER.length]][1];
  ...
  btn.addEventListener('click', () => {
    setTheme(ORDER[(ORDER.indexOf(root.getAttribute('data-theme')) + 1) % ORDER.length]);
  });
