// ═══════════════════════════════════════════════════════════
// СПАСИБО ENGINE v0.8
// Mechanics only. Content lives in scenes.js.
// ═══════════════════════════════════════════════════════════
...
function drawAtmos() {
  ...
  const fogMult = mood==='tense'?2.5:(mood==='uncanny'?0.6:(mood==='revelation'?0.3:1));
  for (const p of fogParts) {
    ...
    const op=(p.base+Math.sin(p.ph)*.012)*fogMult;
    const g=ctx.createRadialGradient(p.x,p.y,0,p.x,p.y,p.r);
    g.addColorStop(0,`rgba(${fr|0},${fg|0},${fb|0},${op.toFixed(3)})`); g.addColorStop(1,`rgba(${fr|0},${fg|0},${fb|0},0)`);
  ...
  if(mood==='tense') li+=Math.sin(T*4.5)*.03;
  if(mood==='revelation') li=.28; if(mood==='uncanny') li=.04;
  lg.addColorStop(0,`rgba(176,120,40,${li.toFixed(3)})`); lg.addColorStop(1,'rgba(176,120,40,0)');
...
function drawPorthole() {
  ...
  ctx.strokeStyle=`rgba(60,100,130,${mood==='uncanny'?0.5:0.25})`; ctx.lineWidth=.8;
  ...
  ctx.restore();
  ctx.strokeStyle='#2a3c48'; ctx.lineWidth=1.5; ctx.beginPath(); ctx.arc(x,y,r*.87,0,Math.PI*2); ctx.stroke();
}
drawAtmos();

// ── PERSISTENCE ─────────────────────────────────────────────
const SAVE_KEY='spasibo_save', PLAY_KEY='spasibo_plays', FLAGS_KEY='spasibo_past_flags';
...
let G = {
  ...
  scene:'chapel_waking',
  lastReaction:null,
  panelOpen:null, confirmRestart:false,
  playCount:parseInt(localStorage.getItem(PLAY_KEY)||'0'),
  pastFlags:JSON.parse(localStorage.getItem(FLAGS_KEY)||'[]'),
};
...
function saveGame() {
  try {
    localStorage.setItem(SAVE_KEY,JSON.stringify({stats:G.stats,charisms:G.charisms,soundings:G.soundings,cover:G.cover,notes:G.notes,flags:[...G.flags],scene:G.scene,mode:G.mode,lastReaction:G.lastReaction}));
...
function loadGame() {
  try {
    ...
    G.cover=s.cover||G.cover; G.notes=s.notes||[];
    G.flags=new Set(s.flags||[]); G.scene=s.scene||'chapel_waking';
    G.mode=s.mode||'attended'; G.lastReaction=s.lastReaction||null;
    G.phase='game'; return true;
...
// ── SOUNDINGS ────────────────────────────────────────────────
// FIX: limit counts taken + settled together (4 total, ever)
...
function renderTitle(root) {
  ...
  root.innerHTML=`<div class="title-screen">
    ...
    <div style="font-size:.68rem;color:var(--amber-dim);letter-spacing:.18em;font-style:italic;margin-bottom:3.5rem">Thank You</div>
  ...
  ~~~~|______________|~~~~
    ...
    <div style="margin-top:2.5rem;display:flex;gap:1rem">
      <button class="btn btn-sm" onclick="absoluteReset()" style="color:var(--border-mid);font-size:.6rem">reset all crossings</button>
    </div>
  </div>`;
}
function continueGame(){if(loadGame())render();else{G.phase='mode';render();}}

// FIX: absolute reset — clears playCount and pastFlags to zero
function absoluteReset() {
  ...
  G={phase:'title',mode:'attended',stats:{vigilance:0,composure:0,communion:0,doubt:0},charisms:[],cover:{posting:null,background:null,denomination:null,connection:null,left:null},soundings:{available:[],taken:[],settled:[]},soundingPending:null,notes:[],flags:new Set(),scene:'chapel_waking',lastReaction:null,panelOpen:null,confirmRestart:false,playCount:0,pastFlags:[]};
  render();
}

function renderMode(root) {
  root.innerHTML=`<div class="sel-screen">
    <div class="sel-h">How will you cross?</div>
...
function selCharism(id) {
  const showWaking=G.playCount>=2;
  const max=showWaking?2:1;
  if(G.charisms.includes(id)){
    G.charisms=G.charisms.filter(x=>x!==id);
  }else if(G.charisms.length<max){
    G.charisms=[...G.charisms,id];
  }
  render();
}
...
function renderCharism(root) {
  ...
  let html=`<div class="sel-h">A charism.</div>
    ...
    ${showWaking?`<br><span style="color:var(--cold);font-size:.76rem">Something new is available. Choose up to two.</span>`:''}
    </div><div class="sel-section">Sleeping</div>`;
  CHARISMS.sleeping.forEach(c=>{
    html+=`<div class="sel-opt${G.charisms.includes(c.id)?' chosen-cold':''}" onclick="selCharism('${c.id}')">
      <div class="sel-name sel-name-cold">${c.name}</div><div class="sel-desc">${c.desc}</div></div>`;
  });
  if(showWaking){
    html+=`<div class="sel-section sel-section-cold">Waking</div>`;
    CHARISMS.waking.forEach(c=>{
      html+=`<div class="sel-opt${G.charisms.includes(c.id)?' chosen-cold':''}" onclick="selCharism('${c.id}')">
        <div class="sel-name sel-name-cold">${c.name}</div><div class="sel-desc">${c.desc}</div></div>`;
    });
  }
  html+=`<div style="font-size:.7rem;color:var(--dim);text-align:center;margin:.8rem 0">${G.charisms.length}/${max} chosen</div>`;
...
function beginGame(){
  ...
  G.phase='game'; G.scene='chapel_waking'; render();
}
...
function renderGame(root) {
  ...
  Object.entries(G.stats).forEach(([k,v])=>{const d=document.createElement('div');d.className='stat';d.innerHTML=k+' <span class="stat-val">'+v+'</span>';sb.appendChild(d);});
  hdr.appendChild(sb);
  const tags=[];
  ...
  // Scene text (static array or function)
  const rawText=typeof scene.text==='function'?scene.text(G):scene.text;
  ...
  rawText.forEach((raw,idx)=>{
    ...
    if(showAnam&&scene.anamnesis.after===idx){
  ...
  if(G.notes.length){
    ...
    G.notes.slice(-5).forEach(k=>{const d=document.createElement('div');d.className='obs-item';d.textContent=noteLabel(k);od.appendChild(d);});
    body.appendChild(od);
  }
  // Restart
  ...
  const bb=document.createElement('button');bb.className='breviary-btn';bb.textContent='breviary';bb.onclick=()=>openPanel('breviary');root.appendChild(bb);
...
function newPlay() {
  ...
  G.lastReaction=null; G.phase='charism'; G.panelOpen=null; G.confirmRestart=false; render();
}
function doRestart() {
  ...
  G.soundings={available:[],taken:[],settled:[]}; G.soundingPending=null;
  G.cover={posting:null,background:null,denomination:null,connection:null,left:null};
  G.lastReaction=null; G.panelOpen=null; G.confirmRestart=false; G.scene='chapel_waking'; render();
}
...
function renderNotesPanel(root) {
  ...
  if(!G.notes.length){const d=document.createElement('div');d.className='panel-empty';d.textContent='Nothing noted yet.';p.appendChild(d);}
    ...
    // Group by category with bullets
    const categories=[
      {label:'People',keys:k=>k.startsWith('met_')},
      {label:'Cover',keys:k=>k.startsWith('cover_')},
      {label:'Charisms',keys:k=>k.startsWith('charism_')},
      {label:'Events',keys:k=>!k.startsWith('met_')&&!k.startsWith('cover_')&&!k.startsWith('charism_')},
    ];
    const shown=new Set();
    categories.forEach(cat=>{
      const items=[...G.notes].reverse().filter(k=>cat.keys(k)&&!shown.has(k));
...
function renderStatusPanel(root) {
  ...
  const addRow=(a,b,bc)=>{const r=document.createElement('div');r.className='stat-row';r.innerHTML=`<span class="stat-label">${a}</span><span class="stat-val-panel" style="color:${bc||'var(--bright)'}">${b}</span>`;p.appendChild(r);};
  ...
  addSec('Cover');
  Object.entries(G.cover).forEach(([k,v])=>addRow(cl[k]||k,v?v.replace(/_/g,' '):'—',v?'var(--amber)':'var(--dim)'));
...
function renderBreviaryPanel(root) {
  ...
  if(G.soundingPending){
    ...
    warn.className='breviary-warn';
    warn.textContent='Breviary full. To take "'+pd.name+'", release a sounding below.'; p.appendChild(warn);
  }
  if(G.soundings.available.length){
    ...
    G.soundings.available.forEach(id=>{
      ...
      card.innerHTML='<div class="sounding-name">'+snd.name+'</div><div class="sounding-fragment">'+snd.fragment+'</div><div class="sounding-origin">'+snd.origin+'</div><div class="sounding-body">'+processText(snd.taking)+'</div>';
      ...
      btn.textContent=full?'Breviary full — release a sounding first':'Take this sounding';
  ...
  if(G.soundings.taken.length){
    ...
    G.soundings.taken.forEach(({id,progress})=>{
      ...
      const btn=document.createElement('button'); btn.className='sounding-btn sounding-btn-release';
      btn.textContent='Release (lose permanently)';
      btn.onclick=()=>{if(confirm('Release "'+snd.name+'"?\nThis sounding will be lost.'))releaseSounding(id);};
      card.appendChild(btn); p.appendChild(card);
    });
  }
